...
(:~
 ...
 : in this module will retrieve the xqDoc XML associated with the MarkLogic CTS built-ins
 ...
 : <p/>
 : The following code will create the trigger associated with this 'online' 
...
(: 
 : Check to see if the xqDoc XML for the MarkLogic CTS built-ins  has already
 : been stored in the xqDoc XML database ("xqDoc").  If it already
 : exists, then there is no reason to call the xqDoc web service.
 :)
 if (xdmp:eval-in(concat("if (exists(doc('", $xqws:ML-CTS-URI, "'))) then true() else false()"),
                  xdmp:database($xqdocDb))) then
   ()
 else

(: 
 : Get the XML from the xqDoc Web Service for the MarkLogic CTS
 : built-ins.  If a different version of the CTS built-ins is desired,
 : then change the URI below to the appropriate value. 
 :)
let $xqdocCtsXml := xqws:get-xqdoc-module-xml((), 
                                              $xqws:ML-CTS-3.0-URI,())
...
(: 
 ...
 :)
let $quotedCtsXml := xdmp:quote($xqdocCtsXml)

let $fixedCtsXml := replace($quotedCtsXml, "'", "&apos;")


(: Construct the query for inserting the xqDoc XML into the xqDoc database :)
let $ctsQuery := concat("declare namespace mine='http://xqdoc.org/mine' ", 
                        "define variable $mine:ctsXml as xs:string external ",
...
xdmp:eval-in($ctsQuery, 
             xdmp:database($xqdocDb),
			 (xs:QName("mine:ctsXml"), $fixedCtsXml))
